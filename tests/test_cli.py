...
def test_cli_route_command_verbose(tmp_path: Any, capsys: Any) -> None:
    ...
    with (
        patch("compitum.cli.CalibratedPredictor", MagicMock()),
...
def test_cli_route_command_trace(tmp_path: Any, capsys: Any) -> None:
    """Test the CLI's route command with the --trace flag (prints full certificate)."""
    from compitum.cli import main
    ...
    with (
        ...
        mock_router_instance = MagicMock()
        mock_cert = MagicMock()
        mock_cert.to_json.return_value = '{"model": "mock_model", "drift": {"trust_radius": 1.0}}'
        mock_router_instance.route.return_value = mock_cert
        mock_CompitumRouter.return_value = mock_router_instance
