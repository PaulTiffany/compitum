...
def analyze_router_results(collection, router_type):
    ...
    if router_results:
        ...
        if per_prompt_df is not None:
            ...
            per_prompt_df["is_correct"] = (
                per_prompt_df["chosen_model"] == per_prompt_df["oracle_chosen_model"]
            )
            ...
            print(
                f"Accuracy (based on oracle match): {len(per_prompt_df[per_prompt_df['is_correct'] == True]) / len(per_prompt_df):.4f}"
            )
            ...
            if not incorrect_decisions.empty:
                ...
                print(
                    incorrect_decisions[
                        ["sample_id", "prompt", "chosen_model", "oracle_chosen_model"]
                    ]
                    .head()
                    .to_string()
                )
