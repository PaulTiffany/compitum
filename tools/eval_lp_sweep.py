...
def evaluate_group(
    ...
    state_dim: int,
    rank: int,
    curvature_alarm: float,
    scales: np.ndarray | None,
) -> Tuple[float, float, float]:
    cfg = PlasmaMonitorConfig(
        state_dim=state_dim, rank=rank, curvature_alarm=curvature_alarm, scales=scales, norm_p=p
    )
    ...
    for r in rows:
        if r.crash_index is not None:
            miss_list.append(
                0.0 if (r.alarm_index is not None and r.alarm_index < r.crash_index) else 1.0
            )
...
def main() -> int:
    ...
    ap.add_argument(
        "data_dir", type=Path, help="Directory of shot CSVs (time_ms,q_min[,Te_core,ne,...])"
    )
    ...
    ap.add_argument(
        ...
        help="Comma-separated p values in [1,2]",
    )
    ap.add_argument(
        "--lambda",
        dest="lam",
        type=float,
        default=1.0,
        help="Weight for HS in Omega = HR + lam * HS",
    )
    ...
    rows = []
    for p in ps:
        risk, far, med_lead = evaluate_group(
            args.data_dir, p, args.state_dim, args.rank, args.curvature_alarm, scales
        )
