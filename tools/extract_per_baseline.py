...
for base_type in ['svm', 'mlp', 'knn', 'cascading router']:
    ...
    for w in sorted(bsub['willingness_to_pay'].dropna().unique()):
        ...
        if C.empty or B.empty:
            continue
        # Merge on eval_name
        M = pd.merge(C[['eval_name','performance']], B[['eval_name','performance']], on='eval_name', suffixes=('_comp','_base'))
        if len(M) == 0:
            continue
        win_rate = float((M['performance_comp'] >= M['performance_base']).mean())
        rows.append({'baseline': base_type, 'wtp': float(w), 'compitum_vs_baseline_win_rate': win_rate, 'n': int(len(M))})
