...
def main() -> int:
    ...
    ap.add_argument(
        "--kmeans", type=int, default=0, help="If >0, k-means cluster SRMF into K layers"
    )
    ap.add_argument(
        "--quantile-layer-on",
        type=str,
        default="band_gap",
        help="Column to quantile-bin into layers",
    )
    ap.add_argument(
        "--quantiles", type=str, default="0.0,0.5,1.0", help="Quantile edges (e.g., 0.0,0.5,1.0)"
    )
    ...
    summary["quantile_layers"] = {
        r["layer"]: {"size": r["size"], "AURC": r["AURC"]} for r in records
    }
    ...
    if args.kmeans and args.kmeans > 0:
        try:
            ...
            summary["kmeans_layers"] = {
                r["layer"]: {"size": r["size"], "AURC": r["AURC"]}
                for r in records
                if str(r["layer"]).startswith("kmeans:")
            }
    ...
    summary["phase_layers"] = {
        r["layer"]: {"size": r["size"], "AURC": r["AURC"]}
        for r in records
        if str(r["layer"]).startswith("phase:")
    }
