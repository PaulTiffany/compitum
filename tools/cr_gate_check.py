...
def main() -> int:
    ...
    ap.add_argument("--gate", action="store_true", help="Fail (exit 1) when score is below threshold")
    args = ap.parse_args()

    try:
        data = json.loads(args.summary_json.read_text(encoding="utf-8"))
    except FileNotFoundError:
        print(f"Cosmic Ray (quick shard) {args.group}: summary file missing at {args.summary_json}", file=sys.stderr)
        return 0 if not args.gate else 1

    score = float(data.get("mutation_score", 0))
    print(f"Cosmic Ray (quick shard) {args.group} score: {score} threshold: {args.threshold} gate: {args.gate}")
