...
def main() -> int:
    ...
    all_csvs = sorted(
        base.glob("eval_results-eval-all-*-val_split.csv"),
    ...
    def choice_cols(prefix: str):
        return sorted(
            [
                ...
                and ("|willingness_to_pay:" in c)
                and not c.endswith("|total_cost")
            ]
        )
    ...
    for base_prefix in ["svm|", "mlp|", "knn|", "cascading router|"]:
        ...
        shared_wtps = sorted(
            set([k for k in bmap.keys() if k is not None])
            & set([k for k in cmap.keys() if k is not None])
        )
        if not shared_wtps:
            continue
    ...
    Path("reports/per_baseline_agreement.json").write_text(
