"""Tranche 2 FabricPC observer: the declared 17-channel input, not a toy clamp.

Runs only under ``.venv-fabricpc``. Imports FabricPC read-only against the
same pinned checkout/receipt as tranche 1
(``experiments/fabricpc/fabricpc_install_receipt.json``); never patches or
forks it. Graph: source (IdentityNode, dim=CHANNEL_DIMENSION) -> hidden
(Linear, dim=8) -> latent (Linear, dim=4). The target/latent clamp is a
fixed, meaningless constant (zeros) purely to force genuine settling
dynamics -- tranche 1 already established that an unsupervised (input-only
-clamped) feedforward graph sits exactly at its equilibrium, producing a
trivially constant trajectory. The network's PARAMETERS are fixed once per
sequence (seeded from the sequence id); only the clamped source channel
vector evolves step to step, so the observed trajectory reflects how a
fixed network reconciles the evolving declared Compitum state, not a
changing network.
"""
...
from compitum.constraint_oracle.channels import CHANNEL_DIMENSION  # noqa: E402
...
def observe_channel_vector(
    ...
    """One lightweight-history observation of one step's channel vector.

    The network's parameters are seeded from ``sequence_id`` alone (fixed
    for the whole sequence); ``step`` only labels the output, it does not
    affect the seed, so consecutive steps genuinely share one network.
    """
    ...
    return {
        ...
        "config": {
            ...
            "graph": "source(17)->hidden(8)->latent(4), InferenceSGD, x and y clamped",
            "history": "lightweight (run_inference_with_history)",
            "channel_layout": "compitum.constraint_oracle.channels (17-dim, see its docstring)",
        },
