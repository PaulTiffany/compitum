...
def compute_alarm_series(monitor: PlasmaMonitor, time_ms: np.ndarray, state_seq: np.ndarray
...
def lead_time_from_q_threshold(q_min: np.ndarray, time_ms: np.ndarray,
                               alarm_idx: Optional[int], q_threshold: float = 1.0
...
def evaluate_shot_csv(path: Path, monitor: PlasmaMonitor,
                      state_dim: int = 8) -> LeadTimeResult:
    ...
    lead_ms, crash_idx = lead_time_from_q_threshold(data.q_min, data.time_ms, alarm_idx,
                                                   q_threshold=monitor.q_threshold)
    return LeadTimeResult(shot_id=path.stem,
                          alarm_index=alarm_idx,
                          crash_index=crash_idx,
                          lead_time_ms=lead_ms)


def evaluate_dir_csv(dir_path: Path, monitor_cfg: Optional[PlasmaMonitorConfig] = None,
                     state_dim: int = 8) -> pd.DataFrame:
